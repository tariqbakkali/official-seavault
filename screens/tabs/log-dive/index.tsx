...
export default function LogDiveScreen() {
  ...
  const [customDiveSiteName, setCustomDiveSiteName] = React.useState('');
  ...
  const handleCustomDiveSiteCreate = () => {
    if (!customDiveSiteName.trim()) {
      Alert.alert('Error', 'Please enter a dive site name');
      return;
    }

    // Create a new dive site object
    const newDiveSite: DiveSite = {
      id: `custom_${Date.now()}`,
      name: customDiveSiteName.trim(),
      latitude: null,
      longitude: null,
      osm_id: null,
    };

    // Add to dive sites list
    setDiveSites(prev => [...prev, newDiveSite]);
    
    // Select this new site
    setSelectedDiveSite(newDiveSite);
    
    // Clear the custom name field
  ...
  const handleGetCurrentLocation = async () => {
    ...
    try {
      ...
      // Create a new dive site with current location
      const newDiveSite: DiveSite = {
        id: `gps_${Date.now()}`,
    ...
    <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.manualDiveSiteContainer}>
            <TextInput
              style={styles.input}
            ...
            />
            <TouchableOpacity
              style={styles.createDiveSiteButton}
              onPress={handleCustomDiveSiteCreate}
            >
              <Text style={styles.createDiveSiteButtonText}>Create</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  manualDiveSiteContainer: {
    flexDirection: 'row',
    gap: 8,
  },
  createDiveSiteButton: {
    backgroundColor: '#007AFF',
    paddingHorizontal: 16,
    paddingVertical: 12,
    borderRadius: 8,
